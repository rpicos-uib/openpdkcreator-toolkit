...
**Planes**, **Contacts**, **Aliases**, and **Styles**, the five Magic
Tech domains with a real form (see ``gui/magic_tech_view.py``'s own
docstring). Every other real section (compose/connect/cifoutput/
cifinput/drc/extract/everything ``magic_tech.py`` doesn't parse) is
copied verbatim, untouched -- there's no editor for them, so nothing
to write back; deliberately bounded, not attempted for all eleven
remaining real sub-tabs at once (see README's own Future Work note on
the remaining gap).

Each real entry in all five editable domains occupies exactly one real
line (``[-]plane name,alias1,alias2`` for a type; ``NAME, SHORT`` for a
plane; ``TYPE LAYER1 LAYER2`` for a contact; ``NAME MEMBERS`` for an
alias; ``TYPE_NAME STYLE1 STYLE2 ...`` for a style -- no block
structure to navigate, unlike LEF's ``PIN``/``MACRO`` or DRC's
multi-line ``.output()`` calls), so all five share one generic patch
routine, ``_render_section_patch``, parameterized by a per-domain
line-render function -- rather than five near-duplicated copies of the
same real positional-patch logic.
``pdklib/magic_tech.py``'s own ``TypeEntry.line_no``/``PlaneEntry.
line_no``/``ContactEntry.line_no``/``AliasEntry.line_no``/
``StyleEntry.line_no`` (tracked at parse time, only when safely
mappable back to real file line numbers -- see
``_safe_prefix_line_count``'s own docstring) are used exactly
...
's no unmodeled per-entry content to lose in any of the five (unlike a
...
models. Styles' own real section additionally carries one, real
non-entry ``styletype NAME`` header line right after its opening
...
``all_line_nos`` in the first place.

All five real sections (confirmed real, not assumed: ``planes``
83-98, ``types`` 104-260, ``contact`` 266-298, ``aliases`` 304-382,
``styles`` 388-529 in IHP's own real ``ihp-sg13g2.tech``) sit well
before the file's own first real ``include`` line, so all five share
the exact same real ``safe_through`` boundary already established for
Types alone. If a given domain's own section start/end line is ``0``
(not found at a safely-mappable position), that one domain's real
lines are simply left untouched -- refusing to guess is safer than
patching the wrong real position -- while the other four still patch
normally.
"""
