...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{cifoutput}: every real \texttt{layer NAME \ldots calma L D}
    recipe -- \texttt{NAME} here is the layer's own CIF/GDS-side
    label (e.g.\ \texttt{BE\_Au}), not the Magic type feeding it, so
    this is a genuine CIF-layer-name to GDS-layer/datatype mapping,
    cross-referenced against the KLayout \texttt{.lyp} layers (keyed
    by that same CIF-side name) in
    ...
    reason to paint -- a sensible finding, not a parsing gap). A later
    pass (\texttt{CifOutputLayerMapping.recipe\_types}) additionally
    recovers the real Magic type name(s) on that same \texttt{layer}
    line, but only for the bare pass-through case with no real
    geometry recipe in between -- 38/138 real entries in IHP's own
    deck; the rest are correctly left unresolved rather than guessed
    through an unmodeled recipe.
...
The toolkit's original scope was deliberately bounded to reading,
cross-referencing, and editing an \emph{existing} real PDK's
structured content; from-scratch generation (a novel technology file,
a new DRC deck, a new device model) has since been added
(\texttt{File} $\to$ \textbf{Create a New/Blank PDK...}, each native
format's own \texttt{create\_new\_*} skeleton writer) and exercised
end-to-end building a genuinely independent, from-scratch memristor
PDK -- its own technology file, DRC deck, xschem symbols/schematics,
Magic layouts, and three distinct Verilog-A compact device models,
verified through real Magic extraction, LVS, DRC, and \texttt{ngspice}
simulation of the extracted layout. Concretely still open: the much
larger, structurally irregular remainder of
