...
The tool is organized around three layers: \texttt{ihp/} (one module
per native format, each exposing a real, typed parse of that
format's structured content), \texttt{gui/} (a Tk application whose
tabs mirror the \texttt{ihp/} module boundaries, grouped by what they
...
The \textbf{By Cell} view (\texttt{ihp/cells.py}, \texttt{gui/cell\_hub\_view.py})
...
\texttt{ihp/layers.py} parses KLayout's \texttt{.lyp} layer-properties
...
line-range write-back in \texttt{ihp/layers\_writer.py}, verified
...
\texttt{ihp/magic\_tech.py} parses Magic's \texttt{.tech} format,
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{cifoutput}: every real \texttt{layer NAME \ldots calma L D}
    ...
    \texttt{ihp/reconcile.py} (100/377 real GDS layer/datatype pairs
...
\texttt{ihp/lef.py} parses tech-LEF layers, \texttt{SITE}s, and real
...
write-back path (\texttt{ihp/lef\_writer.py}).

\subsection{KLayout DRC decks}

\texttt{ihp/drc.py} extracts two reliable patterns from the real
DRC-deck Ruby DSL --
...
\texttt{ihp/liberty.py} extracts real per-pin and per-timing-arc
...
\texttt{ihp/gds.py} provides real structural summaries -- bounding
...
\texttt{ihp/netlist.py} and \texttt{ihp/verilog.py} extract real CDL/SPICE
...
via a shared port editor. \texttt{ihp/spice\_models.py} extracts real
...
inside a \texttt{.subckt} body). \texttt{ihp/xschem.py} extracts a
...
(\texttt{ihp/netlist\_writer.py}, \texttt{ihp/verilog\_writer.py}) has
